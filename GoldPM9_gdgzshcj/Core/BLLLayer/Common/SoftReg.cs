...
namespace Common
{
    public class SoftReg
    {
        ...
        public string GetMd5(object text)
        {
            ...
            Byte[] buffer = System.Text.Encoding.GetEncoding("utf-8").GetBytes(text.ToString());
    ...
    public class SoftRemind
    {
        private readonly string KV = "1234567812345678";
        public string GetRegCode(List<string> ListStr)
        {
            string ServerCode = new SoftReg().getMNum();
            ListStr.Add(ServerCode);

            var AESStr = string.Join(",", ListStr);

            return AESHelper.Encrypt(AESStr, KV, KV);
        }

        public string[] GetRegList(string SoftWareRegistration)
        {
            string arr = "";
            try
            {
                arr = AESHelper.Decrypt(SoftWareRegistration, KV, KV);
            }
            catch { return null; }
            return arr.Split(',');
        }

...
{
    [Description("SafeManage")]
    public class SafeManageController : CoreController
    {
        ...
        public ActionResult list()
        {
            ...
            ViewBag.permission = JavaScriptSerializerUtil.getJson((PermissionList("SafeManage")));
        ...
        public string json()
        {
            ...
            DataTable list = safeBus.GetList(PageModel.PageSize, PageModel.PageIndex, PageModel.PredicateValue != null ? PageModel.PredicateValue[0].ToString() : "", "Id DESC", out recordcount);
        ...
        {
            SafeManageData safeBus = new SafeManageData();
            var flowIds = new DBSql.PubFlow.Flow().GetList(p => p.Id > 0).Where(p => p.FlowRefTable == "SafeManage" && id.Contains(p.FlowRefID)).Select(p => p.Id);
        ...
        {
            SafeManageData safeBus = new SafeManageData();
            SafeManage model = new SafeManage();

            if (Id > 0)
            {
                model = DTHelper.GetEntity<SafeManage>(safeBus.Get(Id));
            }
            model.MvcSetValue();

...
{
    public class ProjectProcessor : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            else
            {
                ...
                foreach (DataModel.Models.Project p in list)
                {
                    ...
                    var bd = new DBSql.Sys.BaseData().GetList(x => projPhaseIDs.Contains(x.BaseID.ToString()));

                    var t = 0;
                    if (projPhaseIDs.Length>0)
                    {
                        foreach (var projPhaseId in projPhaseIDs)
                        {
                            int PhaseId = TypeParse.parse<int>(projPhaseId);
                            var ph = bd.Where(x => x.BaseID.ToString() == projPhaseId).FirstOrDefault();
                            string PhaseName = "";
                            if (ph != null)
                            {
                                PhaseName = ph.BaseName;
                            }

                            int phaseEmpId = p.ProjEmpId;
                            string phaseEmpName = p.ProjEmpName;

                            string ProjNumber = p.ProjNumber;
                            string ProjName = p.ProjName;
                            t++;

                            DateTime CPlanStart = TypeParse.parse<DateTime>(p.DatePlanStart, Convert.ToDateTime("1900-01-01"));
                            DateTime CPlanFinish = TypeParse.parse<DateTime>(p.DatePlanFinish, Convert.ToDateTime("1900-01-01"));
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    string ProjPhaseGridData = Request.Params["HiddenProjPhaseGridData"].ToString().Trim(',');

                    List<DataModel.Models.Project> listProjPhase = new JavaScriptSerializer().Deserialize<List<DataModel.Models.Project>>(ProjPhaseGridData);
                    DataModel.EmpSession userInfo = Session["Employee"] as DataModel.EmpSession;//获取当前登录的用户
                    ...
                    List<string> listGather = new List<string>();
                    ...
                    {
                        DBSql.Design.DesTaskGroup desTaskGroupDB = new DBSql.Design.DesTaskGroup();
                        desTaskGroupDB.ModifyProjectAndTaskGroup(model.Id);
                    });

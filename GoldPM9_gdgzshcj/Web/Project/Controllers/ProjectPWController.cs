...
{
    [Description("ProjectPW")]
    public class ProjectPWController : CoreController
    {
        private DBSql.Project.Project dbProject = new DBSql.Project.Project();
        private DBSql.Project.ProjectAuxiliary dbProjectAuxiliary = new DBSql.Project.ProjectAuxiliary();
        private DBSql.Sys.BaseData baseDataDB = new DBSql.Sys.BaseData();
        private DoResult doResult = DoResultHelper.doError("未知错误!");

        #region 列表
        [Description("配网项目登记(分公司)")]
        public ActionResult list()
        {
            ViewBag.CurrentEmpID = userInfo.EmpID;
            ViewBag.permission = JavaScriptSerializerUtil.getJson((PermissionList("ProjectPW")));
            return View();
        }

        [Description("配网项目登记(设计公司)")]
        public ActionResult list_sj()
        {
            ViewBag.CurrentEmpID = userInfo.EmpID;
            ViewBag.permission = JavaScriptSerializerUtil.getJson((PermissionList("ProjectPW_SJ")));
            return View();
        }

        [Description("配网项目登记(工程公司)")]
        public ActionResult list_gc()
        {
            ViewBag.CurrentEmpID = userInfo.EmpID;
            ViewBag.permission = JavaScriptSerializerUtil.getJson((PermissionList("ProjectPW_GC")));
            return View();
        }

        [Description("配网项目登记(创景公司)")]
        public ActionResult list_cj()
        {
            ViewBag.CurrentEmpID = userInfo.EmpID;
            ViewBag.permission = JavaScriptSerializerUtil.getJson((PermissionList("ProjectPW_CJ")));
            return View();
        }
        #endregion
        ...
        public string jsonNew(int category = 0)
        {
            ...
            List<string> result = PermissionList("ProjectPW");
            Common.SqlPageInfo PageModel = new Common.SqlPageInfo();
            if (!PageModel.SelectOrder.isNotEmpty())
            {
            ...
            string refTable = "'ProjectPW'";
            switch (companyType)
            {
                case "SJ":
                    PageModel.SelectCondtion.Add("CompanyID", 1);
                    result = PermissionList("ProjectPW_SJ");
                    refTable = "'ProjectPW_SJ'";
                    break;
                case "GC":
                    PageModel.SelectCondtion.Add("CompanyID", 2);
                    result = PermissionList("ProjectPW_GC");
                    refTable = "'ProjectPW_GC'";
                    break;
                case "CJ":
                    PageModel.SelectCondtion.Add("CompanyID", 3);
                    result = PermissionList("ProjectPW_CJ");
                    refTable = "'ProjectPW_CJ'";
                    break;
                default:
                    PageModel.SelectCondtion.Add("CompanyID", 0);
                    result = PermissionList("ProjectPW");
                    refTable = "'ProjectPW'";
                    break;
            }
        ...
        public ActionResult add()
        {
            ...
            var ProjTypeIds = DbBaseData.getTreeData(DbBaseData.GetBaseDataInfoByEngName("ProjectType"), 0, ((int)ProjectTypeModel.配网).ToString());
        ...
        /// </summary>
        /// <returns></returns>
        public object ExcelImportProject(string filePath)
        {
            //项目负责人
            ...
            //类型
            var ProjTypeIds = from m in new DBSql.Sys.BaseData().GetDataSetByEngName("ProjectType") where m.BaseOrder.StartsWith("003_006_002_") select new { value = string.Format("{0}", m.BaseID), text = m.BaseName };      
            //阶段
            var ProjPhaseIds = from m in new DBSql.Sys.BaseData().GetDataSetByEngName("ProjectPhase") select new { value = string.Format("{0}", m.BaseID), text = m.BaseName };
            //电压
            ...
            //序号 子项代码 子项编号 子项名称 子项阶段 子项类型  电压等级	回路数	线路长度(km)	变压器容量(MVA) 台数 设总 投资额 子项内容说明 备注  立项时间  计划开始时间 计划完成时间

            Dictionary<string, string> ColMap = new Dictionary<string, string>();
            //tableCol.Add("子项代码", "");                 //////////////////////
            ColMap.Add("子项编号", "ProjNumber");
            ColMap.Add("子项名称", "ProjName");
            ColMap.Add("子项阶段", "ProjPhaseIds");
            ColMap.Add("子项类型", "ProjTypeID");
            ...
            #region 验证列数是否都存在
            Msg = "文件中不存在列名：";
            foreach (var ColItem in ColMap)
            {
                if (!dt.Columns.Contains(ColItem.Key))
                {
                    errList.Add(string.Format("【{0}】", ColItem.Key));
                }
            }
            if (errList.Count > 0)
            {
                return new { Result = false, Message = string.Join("、", errList.ToArray()) };
            }
            #endregion
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string ErrStr = "";
                #region
                if (dr["子项编号"].ToString().Trim() == "")
                {
                    ErrStr += "子项编号不能为空、";
                }
                if (dr["子项名称"].ToString().Trim() == "")
                {
                    ErrStr += "子项名称不能为空、";
                }

                string phaseName = dr["子项阶段"].ToString();//判断工程阶段
                var _phaseName = ProjPhaseIds.Where(q => ("," + phaseName + ",").Contains("," + q.text.ToString() + ",")).Select(q => q.value);
                if (_phaseName.Count() == 0)
                {
                    dr["子项阶段"] = ProjPhaseIds.FirstOrDefault(p => p.text.Contains("施工图")).value;
                }
                else
                {

                    dr["子项阶段"] = string.Join(",", _phaseName);
                }

                string typeName = dr["子项类型"].ToString();//判断工程阶段
                var _typeName = ProjTypeIds.FirstOrDefault(p => p.text == typeName);
                if (_typeName == null)
                {
                    ErrStr += typeName + ":无法在系统中获取项目类型";
                }
                if (ErrStr != "")
                {
                    errList.Add(string.Format("第{0}行：{1}", Index, ErrStr));
                    continue;
                }

                string VoltName = dr["电压等级"].ToString();//项目负责人
                var _VoltName = ProjVoltIds.FirstOrDefault(p => p.text == VoltName);
                if (_VoltName == null)
                {
                    ErrStr += VoltName + ":无法在系统中获取电压等级";
                }
                if (ErrStr != "")
                {
                    errList.Add(string.Format("第{0}行：{1}", Index, ErrStr));
                ...
                string CircuitsName = dr["回路数"].ToString();//回路数
                var _CircuitsName = ProjCircuits.FirstOrDefault(p => p.text == CircuitsName);
                if (_CircuitsName == null)
                {
                    ErrStr += CircuitsName + ":无法在系统中获取回路数";
                }
                if (ErrStr != "")
                {
                    errList.Add(string.Format("第{0}行：{1}", Index, ErrStr));
                ...
                string EmpName = dr["设总"].ToString();//项目负责人
                var EmpModel = ProjRespEmps.FirstOrDefault(p => p.EmpName == EmpName);
                if (EmpModel == null)
                {
                    ErrStr += EmpName + ":无法在系统中获取设总人员";
                }
                if (ErrStr != "")
                {
                    errList.Add(string.Format("第{0}行：{1}", Index, ErrStr));
                ...
                string investFee = dr["投资额"].ToString();
                try
                {
                    dr["投资额"] = Convert.ToDecimal(investFee).ToString("f4");
                }
                catch
                {
                    dr["投资额"] = "0.0000";
                ...
                //立项时间
                dr["立项时间"] = Common.ExtensionMethods.Value<DateTime>(dr["立项时间"]).Year.ToString() == "1" ? new DateTime(1900, 1, 1).ToString("yyyy/MM/dd") : Common.ExtensionMethods.Value<DateTime>(dr["立项时间"]).ToString("yyyy/MM/dd");
                //计划开始时间
                dr["计划开始时间"] = Common.ExtensionMethods.Value<DateTime>(dr["计划开始时间"]).Year.ToString() == "1" ? new DateTime(1900, 1, 1).ToString("yyyy/MM/dd") : Common.ExtensionMethods.Value<DateTime>(dr["计划开始时间"]).ToString("yyyy/MM/dd");
                //计划完成时间
                dr["计划完成时间"] = Common.ExtensionMethods.Value<DateTime>(dr["计划完成时间"]).Year.ToString() == "1" ? new DateTime(1900, 1, 1).ToString("yyyy/MM/dd") : Common.ExtensionMethods.Value<DateTime>(dr["计划完成时间"]).ToString("yyyy/MM/dd");
            ...
            if (errList.Count > 0)
            {
                return new { Result = false, Message = string.Join("<br />", errList.ToArray()) };
            }
            #endregion
            ...
            {
                //try { p.SetField("子项阶段", string.Join(",", ProjPhaseIds.Where(q => ("," + p.Field<string>("子项阶段") + ",").Contains("," + q.text.ToString() + ",")).Select(q => q.value))); } catch { }
                try { p.SetField("子项类型", ProjTypeIds.Where(q => q.text.ToString() == p.Field<string>("子项类型")).FirstOrDefault().value); } catch { }
            ...
            return new { Result = true, Message = "导入成功!", data = JavaScriptSerializerUtil.getJson(listProject) };
        }
        ...
        public JsonResult UploadTemplate()
        {
            ...
            if (extension == ".xls" || extension == ".xlsx")
            {
                ...
                return Json(ExcelImportProject(templatePath), JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        #region 编辑
        [Description("编辑")]
        public ActionResult perfect(int ProjId)
        {
            var model = dbProjectAuxiliary.GetQuery(p => p.ProjId == ProjId).FirstOrDefault();
            if (model == null)
            {
                model = new DataModel.Models.ProjectAuxiliary();
            }
            ViewBag.ProjId = ProjId;
            return View("Auxiliary", model);
        }
        #endregion
        ...
        [Description("保存")]
        [HttpPost]
        public ActionResult savePerfect(int Id)
        {
            int result = 0;
            var model = new DataModel.Models.ProjectAuxiliary();
            try
            {
                //页面model 项目信息
                ...
                if (Id == 0)
                {
                    ...
                    model.AuxiliaryUnit = Request.Form["AuxiliaryUnit"];
                    model.KCNum = TypeHelper.parseDecimal(Request.Form["KCNum"]);
                    model.AKCNum = TypeHelper.parseDecimal(Request.Form["AKCNum"]);
                    model.SGJH = TypeHelper.parseDateTime(Request.Form["SGJH"]);
                    model.SGWCJH = TypeHelper.parseDateTime(Request.Form["SGWCJH"]);
                    model.DDJH = TypeHelper.parseDateTime(Request.Form["DDJH"]);
                    model.DDWCNum = TypeHelper.parseDecimal(Request.Form["DDWCNum"]);

                    dbProjectAuxiliary.Add(model);
                    dbProjectAuxiliary.UnitOfWork.SaveChanges();
                    result = model.Id;
                ...
                {
                    model = dbProjectAuxiliary.Get(Id);
                    model.AuxiliaryUnit = Request.Form["AuxiliaryUnit"];
                    model.KCNum = TypeHelper.parseDecimal(Request.Form["KCNum"]);
                    model.AKCNum = TypeHelper.parseDecimal(Request.Form["AKCNum"]);
                    model.SGJH = TypeHelper.parseDateTime(Request.Form["SGJH"]);

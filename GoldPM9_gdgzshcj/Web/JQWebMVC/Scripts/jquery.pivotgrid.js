...
(function ($) {
    ...
    function buildFilterBar(target, rows) {
        ...
        $.map(opts.pivot.filters, function (field) {
            ...
            var f = $('<input style="width:200px;">').attr('name', field).appendTo(bar);
            f.combobox({
                multiple: true,
                prompt: '请选择',
            ...
            f.parent().children("span").eq(1).children('span').children('a').eq(0).attr("style", "padding-top:7px;");
            function handler1() {
            ...
            function handler2() {
                ...
                $(this).combobox('setText', values.length ? (values.length == 1 ? values[0] : values.join(',')) : '');
            }
            var ss = document.getElementsByClassName("textbox-icon fa fa-search");
            if (ss != undefined) {
                if (ss.length > 1) {
                    for (var i = 1; i < ss.length; i++) {
                        ss[i].style.lineHeight = "27px";
                    }
                }
            }
        });
    ...
    function layout(target) {
        ...
        if (!state.layoutDialog) {
            var content = '<div id="Lay" class="easyui-layout" fit="true">' +
					'<div region="west" split="false"  border="true" class="pg-fields" title="' + opts.i18n.fields + '" style="width:150px;"></div>' +
            ...
            state.layoutDialog.dialog({
                ...
                buttons: [{
                    ...
                    handler: function () {
                        ...
                        if (opts.pivot.values.length == 0) {//王建斌2016-12-15修改，解决所有列表不能同时作为查询条件问题
                            window.top.$.messager.alert("错误",  "不能把所有列同时作为查询条件！", 'error');
                            return;
                        }
                        opts.pivot.filterRules = {}; //王建斌2016-12-15修改，解决统计列发生变化，查询条件记忆的问题
                        $(target).pivotgrid();
                    }
                }, {
        ...
        function fill(p, d) {
            ...
            $.map(d, function (name) {
                ...
                var text = typeof name == 'object' ? (name.field + '<span style="color:#aaa;margin:0 10px">' + showText + '</span>') : name;
                    ...
                    if ($(this).hasClass('pg-values')) {
                        ...
                        if (opts.op == "sum") {
                            ...
                            if (opts.op == "max") {
                                ...
                                if ((opts.op || 'sum') == "normal") {
                                    showText = "";
                                }
                            }
                        }
                        text += '<span style="color:#aaa;margin:0 10px">' + showText + '</span>';
                    }
                    $(source).linkbutton({
                        op2: opts.op,
                        text: text
        ...
        function getFields(type) {
            ...
            state.layoutDialog.find('.pg-' + type + ' .l-btn').each(function () {
                ...
                if (type == 'values' || type == 'filters') { //王建斌2016-12-16 修正
                ...
                if (!state.opMenu) {
                    ...
                    state.opMenu.menu({
                        onClick: function (item) {
                            ...
                            var text = bopts.field + '<span style="color:#aaa;margin:0 10px">' + (textOp) + '</span>';
                            btn.linkbutton({
                                op: op,
                                text: text
                            });
    ...
    $.fn.pivotgrid.defaults = $.extend({}, $.fn.treegrid.defaults, {
        ...
        frozenColumns: [[
			{ field: '_tree_field', width: 120, title: '', halign: 'center', sortable: true }
        ]],
        autoRowHeight: false,
        remoteSort: false,

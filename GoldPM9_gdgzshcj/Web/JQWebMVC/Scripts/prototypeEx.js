...
// 设置签名控件
DomUtil.setSignControl = function (data, div, userName, date) {
    var signControls;
    if (data != "") {
        signControls = data.split(";");
        for (var i = 0; i < signControls.length; i++) {
            var match = /^([\w\W]*)\(([\w\W]*)\)$/g.exec(signControls[i]);
            if (match && match.length > 0) {
                var ctl = document.getElementById(match[1]);
                if (ctl) {
                    switch (ctl.tagName.toLowerCase()) {
                        case "input":
                            switch (ctl.type.toLowerCase()) {

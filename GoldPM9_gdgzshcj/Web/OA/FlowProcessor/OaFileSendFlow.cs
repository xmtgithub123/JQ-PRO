...
{
    class OaFileSendFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            this.Options.Message = "发文部门[" + model.OaFileSendDep + "],文件名称:" + model.OaFileName;
        ...
        private DataModel.Models.OaFileSend model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.OaFileSend>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    model.OaFileSendNote = Request.Form["OaFileSendNote"];
                    if (model.Id == 0)
                    {
                        this.CurrentDbContext.Set<DataModel.Models.OaFileSend>().Add(model);
                    ...
                    this.Options.Title = "发文部门[" + model.OaFileSendDep + "],文件名称:" + model.OaFileName;
                    break;
            }
        }
    }

...
{
    class OaEquipUseOfficeFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            {
                var originalValues = this.CurrentDbContext.Set<DataModel.Models.OaEquipStock>().Where(m => m.EquipFormID == model.Id && m.EquipFormType == "OaEquipUseOfficeFlow");
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    if (EquipId != "")
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            if (stock == null)
                            {
                                ...
                                stock.EquipNote = "办公用品领用";
                                stock.MvcDefaultSave(this.Options.CurrentUser);
                                stock.EquipFormType = "OaEquipUseOfficeFlow";
                        ...
                        var delModel = this.CurrentDbContext.Set<DataModel.Models.OaEquipStock>().Where(p => p.EquipFormID == model.Id && p.EquipFormType == "OaEquipUseOfficeFlow" && !ids.Contains(p.Id));
                    ...
                    this.Options.Title = model.CreatorEmpName + "的办公用品领用申请单";
                    break;
            }
        }

...
{
    class IsoSJPSJYBFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            else
            {
                ...
                #region 更新项目动态及日志
                var _isoForm = this.CurrentDbContext.Set<DataModel.Models.IsoSJPSJYB>().FirstOrDefault(m => m.Id == this.Options.RefID);
                ...
                foreach (DataModel.Models.DesTaskGroup d in list)
                {
                    ...
                    string logRefHTML = String.Format("[{0}]{1}>设计评审纪要表,已批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DataModel.Models.IsoSJPSJYB model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.IsoSJPSJYB>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    {
                        model = new DataModel.Models.IsoSJPSJYB();
                    ...
                    if (model.Id == 0)
                    {
                        ...
                        this.CurrentDbContext.Set<DataModel.Models.IsoSJPSJYB>().Add(model);
                    }
                    ...
                    this.Options.Title = "[" + model.ProjNumber + "]" + model.ProjName + "的设计评审纪要表";
                    break;
            }
        }
    }

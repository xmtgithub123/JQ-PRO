...
{
    class IsoSJBGDFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            #region 更新项目动态及日志
            var _isoForm = this.CurrentDbContext.Set<DataModel.Models.IsoSJBGD>().FirstOrDefault(m => m.Id == this.Options.RefID);
            ...
            foreach (DataModel.Models.DesTaskGroup d in list)
            {
                ...
                string logRefHTML = String.Format("[{0}]{1}>设计变更单,已批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DataModel.Models.IsoSJBGD model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.IsoSJBGD>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    {
                        model = new DataModel.Models.IsoSJBGD();
                    ...
                    if (model.Id == 0)
                    {
                        ...
                        this.CurrentDbContext.Set<DataModel.Models.IsoSJBGD>().Add(model);
                    }
                    try
                    {
                    ...
                    this.Options.Title = "[" + model.ProjNumber + "]" + model.ProjName + "的设计变更单";
                    break;
            }
        }
    }

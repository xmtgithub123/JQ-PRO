...
{
    class IsoGCDZKCTJDJZFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            #region 更新项目动态及日志
            var _isoForm = this.CurrentDbContext.Set<DataModel.Models.IsoGCDZKCTJDJZ>().FirstOrDefault(m => m.Id == this.Options.RefID);
            ...
            foreach (DataModel.Models.DesTaskGroup d in list)
            {
                ...
                string logRefHTML = String.Format("[{0}]{1}>工程地质勘察条件单(建筑),已批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DataModel.Models.IsoGCDZKCTJDJZ model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.IsoGCDZKCTJDJZ>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    {
                        model = new DataModel.Models.IsoGCDZKCTJDJZ();
                    ...
                    if (model.Id == 0)
                    {
                        ...
                        this.CurrentDbContext.Set<DataModel.Models.IsoGCDZKCTJDJZ>().Add(model);
                    }
                    else
                    {
                        #region 客户单位保存
                    ...
                    this.Options.Title = "[" + model.ProjNumber + "]" + model.ProjName + "的工程测量条件单";
                    break;
            }
        }
    }

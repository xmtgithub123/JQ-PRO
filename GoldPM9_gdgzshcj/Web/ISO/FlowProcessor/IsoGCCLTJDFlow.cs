...
{
    class IsoGCCLTJDFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            #region 更新项目动态及日志
            var _isoForm = this.CurrentDbContext.Set<DataModel.Models.IsoGCCLTJD>().FirstOrDefault(m => m.Id == this.Options.RefID);
            ...
            foreach (DataModel.Models.DesTaskGroup d in list)
            {
                ...
                string logRefHTML = String.Format("[{0}]{1}>工程测量条件单,已批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DataModel.Models.IsoGCCLTJD model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.IsoGCCLTJD>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    {
                        model = new DataModel.Models.IsoGCCLTJD();
                    ...
                    if (model.Id == 0)
                    {
                        ...
                        this.CurrentDbContext.Set<DataModel.Models.IsoGCCLTJD>().Add(model);
                    }
                    else
                    {
                        #region 客户单位保存

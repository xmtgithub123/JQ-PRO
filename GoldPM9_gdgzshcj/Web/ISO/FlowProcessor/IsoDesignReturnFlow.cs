...
{
    public class IsoDesignReturnFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            #region 更新项目动态及日志
            var _isoForm = this.CurrentDbContext.Set<DataModel.Models.IsoDesignReturn>().FirstOrDefault(m => m.Id == this.Options.RefID);
            ...
            foreach (DataModel.Models.DesTaskGroup d in list)
            {
                ...
                string logRefHTML = String.Format("[{0}]{1}>设计回访纪要表,已批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DataModel.Models.IsoDesignReturn model = null;
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        model = this.CurrentDbContext.Set<DataModel.Models.IsoDesignReturn>().FirstOrDefault(m => m.Id == this.Options.RefID);
                        if (model == null)
                        {
                    ...
                    {
                        model = new DataModel.Models.IsoDesignReturn();
                    ...
                    if (model.Id == 0)
                    {
                        ...
                        this.CurrentDbContext.Set<DataModel.Models.IsoDesignReturn>().Add(model);
                    }
                    ...
                    this.Options.Title = "[" + model.ProjNumber + "]" + model.ProjName + "的设计回访纪要表";
                    break;
            }
        }
    }

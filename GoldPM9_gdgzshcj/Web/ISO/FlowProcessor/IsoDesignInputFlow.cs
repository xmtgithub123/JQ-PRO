...
{
    public class IsoDesignInputFlow : IFlowProcessor
    {
        ...
        public void OnAfterApproveFinished()
        {
            ...
            else
            {
                ...
                foreach (DataModel.Models.DesTaskGroup d in list)
                {
                    ...
                    string logRefHTML = String.Format("[{0}]{1}>设计输入评审,被批准", d.ProjNumber, d.TaskGroupName);
        ...
        private DesInputReview desInputReview = new DesInputReview();
        ...
        public void OnSaveForm()
        {
            switch (this.Options.Action)
            {
                ...
                case DBSql.PubFlow.Action.Transfer:
                    ...
                    {
                        this.Options.Title = "设计输入评审";
                    }
                   else
                    {
                        this.Options.Title = "[" + proj.Get(model.ProjID) == null ? "" : proj.Get(model.ProjID).ProjNumber + "]" + proj.Get(model.ProjID) == null ? "" : proj.Get(model.ProjID).ProjName+"--设计输入评审";
                    }
                    break;
            }
        }

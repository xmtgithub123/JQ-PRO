...
{
    [Description("IsoBGSJRWTZD")]
    public class IsoBGSJRWTZDController : CoreController
    {
        ...
        public string json()
        {
            ...
            if (queryInfo.isNotEmpty())
            {
                foreach (Common.Data.Extenstions.Filter item in Filter)
                {
                    ...
                    else if (_child.Key == "ChangeTaskTypeState")
                    {
                        PageModel.SelectCondtion.Add("ChangeTaskTypeState", _child.Value);
                    }
                }
            }
        ...
        public ActionResult del(int[] id)
        {
            ...
            try
            {
                op.UpdateIsoBGSJRWTZDInfo(id, this.userInfo);
                op.UnitOfWork.SaveChanges();

                new DBSql.PubFlow.Flow().Delete(id, "IsoBGSJRWTZD");
                using (var ba = new DBSql.Sys.BaseAttach())
                {
                    ba.Delete(id, "IsoBGSJRWTZD");
                }
                reuslt = 1;
            }
            catch
